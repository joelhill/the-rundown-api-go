...
type EventBySportOptions struct {
	// URL Parts
	URL   string
	Sport string
...
// NewEventBySportOptions - Returns the options with most url parts already set to hit the event by sport endpoint
func (s *Service) NewEventBySportOptions() *EventBySportOptions {
	return &EventBySportOptions{
		URL:   s.Config.BaseURL,
		Sport: s.Config.Sport,
	}
}

// EventBySport - hits the https://therundown-therundown-v1.p.rapidapi.com/sports/3/events endpoint
func (s *Service) EventBySport(options *EventBySportOptions) (TheRunDownAPILines, int, error) {
	...
	err := validateEventBySportURI(options)
	if err != nil {
	...
	uri := fmt.Sprintf("%s/sports/%s/events?cachebuster=%s", options.URL, options.Sport, cacheBuster)
	...
	s.Logger.Debug("EventBySport API Call")
	...
	if err != nil {
		s.Logger.Errorf("something went wrong making the get request for EventBySport: %s", err.Error())
		return mapping, statusCode, err
	}

	s.Logger.Infof("EventBySport Status Code: %d", statusCode)

	if client.StatusCodeIsError() {
		s.Logger.Errorf("EventBySport retuned an unsuccessful status code. Error: %+v", errorPayload)
	}

	return mapping, statusCode, nil
}

func validateEventBySportURI(options *EventBySportOptions) error {
